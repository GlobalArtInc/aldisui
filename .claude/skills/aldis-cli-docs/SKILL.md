...
# Aldis — CLI Documentation Generator
...
(`DEBUG|INFO|WARN|ERROR|FATAL|PANIC`, env `ALDIS_LOG_LEVEL`), `--debug-filter` (DEBUG-only namespace filter,
env `ALDIS_DEBUG_FILTER`). Re-read `Execute()` in `root.go` for the live list.
...
2. A fenced `aldis <group> --help` block.
...
- `cli.md` carries: intro + `aldis help`, a command-group table linking to each page, the **global options**
