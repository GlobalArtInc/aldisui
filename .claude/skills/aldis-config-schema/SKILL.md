---
name: aldis-config-schema
description: >
  Generate or update the JSON Schema (in YAML form) that describes the Aldis configuration file. The schema
...
# Aldis — Config Schema Generator

This skill produces and maintains `config.schema.yaml` — a **JSON Schema draft 2020-12** document (written in
YAML) describing every valid key in a Aldis `config.json` / `config.yaml`. The schema is the source of truth
...
$id: "https://github.com/GlobalArtInc/aldisui"
title: Aldis configuration
description: Schema for Aldis UI config.json / config.yaml. Generated from util.ConfigType.
type: object
```
