---
name: aldis-third-party-licenses
description: >
  Generate or update the THIRD-PARTY-LICENSES.md file for the Aldis UI repository. Use this skill whenever the user 
  asks to create, update, regenerate, audit, or refresh third-party license attributions, OSS notices, SBOM-style 
  license inventories, or NOTICE files for Aldis UI. Trigger on phrases like "third-party licenses", "OSS notices",
  ...
  knows the Aldis UI layout (Go backend + Vue.js frontend), the project's license policy (allowlist/denylist),
  and the canonical output format.
---

# Aldis UI — Third-Party License Generator

This skill produces `THIRD-PARTY-LICENSES.md` for the Aldis UI repository. The file lists every open-source
dependency shipped with Aldis UI by **name**, **version**, **license**, and **source URL**, grouped by component. It
...
Aldis UI is a self-hosted product. The repository ships:
...
processes. They are NOT linked into the binary and NOT redistributed with Aldis UI, so they are intentionally
**not listed** in `THIRD-PARTY-LICENSES.md` — their license terms apply to the user who installs them on the host,
not to Aldis UI's distribution. The license policy still discusses why this is contractually safe (see
...
on the host, not Aldis UI's distribution. Listing them would imply otherwise and is an unnecessary signal to a
