# License Policy for Aldis UI

This is the source of truth for which open-source licenses are acceptable in
Aldis UI dependencies. The policy is enforced by
`scripts/check_policy.py` — if you change anything here, mirror the change
in that script.

The policy exists for two reasons:

1. **Downstream obligations.** Aldis ships under MIT, so it must not bundle
   components on terms that would impose reciprocal licensing on the people
   who deploy it. That rules out copyleft licenses with strong reciprocity
...
without making Aldis UI MPL. If we ever modify an MPL-licensed file,
...
|---------|--------------|
| GPL-2.0, GPL-3.0 (and `-only` / `-or-later`) | Strong copyleft. Linking forces Aldis UI to be GPL, which conflicts with our distribution model. |
...
Tools that Aldis UI invokes via `os/exec` (Ansible, Terraform, OpenTofu,
