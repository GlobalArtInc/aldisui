# Aldis UI Development Instructions

**Always reference these instructions first and fallback to search or bash commands only when you encounter unexpected information that does not match the info here.**

Aldis UI is a modern web interface for managing popular DevOps tools like Ansible, Terraform, PowerShell, and Bash scripts. It's built with Go (backend) and Vue.js (frontend), using Task runner for build automation.
...
- Setup database and admin user: `./bin/aldis setup` (interactive, use SQLite option 4 for development)
- Start server: `./bin/aldis server --config ./config.json`
...
- ALWAYS run through at least one complete end-to-end scenario after making changes:
  ...
  2. Start the server: `./bin/aldis server --config ./config.json`
...
./bin/aldis setup  # Choose option 4 (SQLite), use admin/changeme

# 3. Start server
./bin/aldis server --config ./config.json
...
> **Note:** BoltDB (option 2) was removed in Aldis 2.19. If you have an existing
