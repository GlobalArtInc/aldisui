# Debug Logging in Aldis UI

## Problem

Aldis currently emits a limited set of logs that are good enough for *monitoring*
...
2. **Selectively enable debug output** — be able to turn on debug logs for *specific*
   ...
   library works. Controlled by a new environment variable `ALDIS_DEBUG_FILTER`.

This must be additive and backward compatible: existing behavior (`ALDIS_LOG_LEVEL`,
...
- Log level is configured once in `cli/cmd/root.go` `PersistentPreRun` from the
  `--log-level` flag or `ALDIS_LOG_LEVEL` env var via `log.SetLevel`.
...
that is already in use. `ALDIS_DEBUG_FILTER` selects which namespaces are allowed to
...
### `ALDIS_DEBUG_FILTER` syntax (matches Node `debug`)
...
If `ALDIS_DEBUG_FILTER` is empty/unset, behavior is unchanged from today: debug logs
are emitted only if the global level (`ALDIS_LOG_LEVEL` / `--log-level`) is `DEBUG`,
...
### Interaction with `ALDIS_LOG_LEVEL`

| `ALDIS_LOG_LEVEL` | `ALDIS_DEBUG_FILTER` | Result                                                                 |
|-----------------------|--------------------------|------------------------------------------------------------------------|
...
Rule: `ALDIS_DEBUG_FILTER` only takes effect when the log level is `DEBUG`, and it
...
// Parse builds a Filter from the ALDIS_DEBUG_FILTER value.
...
```go
debugSpec := os.Getenv("ALDIS_DEBUG_FILTER")
...
- Must run **after** the existing `ALDIS_LOG_LEVEL` block so we read the resolved
...
- Document `ALDIS_DEBUG_FILTER` next to `ALDIS_LOG_LEVEL`: env var reference / docs
  site, and the `--help` text if a matching flag is added (optional `--debug-filter`).
- State clearly that the filter requires `ALDIS_LOG_LEVEL=DEBUG` (or `--log-level
  DEBUG`); without it the variable is ignored.
- Provide examples (each assumes the level is `DEBUG`):
    - `ALDIS_LOG_LEVEL=DEBUG ALDIS_DEBUG_FILTER=runner` — debug only the runner subsystem.
    - `ALDIS_LOG_LEVEL=DEBUG ALDIS_DEBUG_FILTER='task_*'` — all task-related namespaces.
    - `ALDIS_LOG_LEVEL=DEBUG ALDIS_DEBUG_FILTER='*,-db'` — everything except database tracing.
...
overrides `ALDIS_DEBUG_FILTER`, resolved the same way (flag wins over env).
...
- No change when `ALDIS_DEBUG_FILTER` is unset.
- No change when the level is above `DEBUG` — the filter is never engaged, so setting the
  env var alone (without `ALDIS_LOG_LEVEL=DEBUG`) has no effect.
- Existing `log.*` call sites are untouched; the formatter wraps the current one.
- `ALDIS_LOG_LEVEL=DEBUG` alone keeps printing everything (filter only narrows the
