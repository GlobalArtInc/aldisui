# OIDC ID Token Authentication implementation plan for Aldis UI

Analog of [GitLab CI/CD ID Tokens](https://docs.gitlab.com/ci/secrets/id_token_authentication/) functionality: the
Aldis server issues short-lived signed JWTs for jobs so they can authenticate to external systems (HashiCorp Vault,
AWS, GCP, Azure) without storing long-lived credentials.

## 1. Concept

The Aldis server becomes an **OIDC provider**: for every task run it issues a short-lived JWT signed with RS256,
with claims about the task/template/project/user. The runner passes this token to Vault / AWS STS / GCP Workload
Identity / Azure to exchange it for short-lived secrets — without storing long-lived credentials in Aldis.
...
- Trust between Aldis and Vault/AWS is set up once: the external side validates `iss` + signature and matches `sub`/
...
- Transparent encryption of the secret with the Aldis master key.
...
- OIDC key metadata (`kid`, `algorithm`, `not_before`, `not_after`, `status`) is stored in a new narrow table
  ...
  duplicated.
- CLI command `aldis oidc rotate-key` + scheduled rotation (cron on 30/90 days). Retiring keys remain available for
...
```
iss   = util.Config.WebHost            (e.g., https://aldis.example.com)
...
- **Issuer URL** must match the real externally-accessible URL of Aldis (`util.Config.WebHost`) — fail-fast on empty
  value.
