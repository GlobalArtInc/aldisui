...
- Round-trip on a real database: create a user with `aldis user add`,
...
| Memory pressure on small Aldis deployments (19 MiB per concurrent login)                                                                           | At our login rate this is negligible. If it ever isn't, the parameter knob is in `pkg/passwordhash` and a future config option is straightforward.                                                                                                                              |
| Downgrade to v2.18 locks out users whose passwords were rehashed                                                                                       | Release notes warn operators to snapshot. The lock-out is recoverable via `aldis user change-by-login --password` (admin-side reset).                                                                                                                                       |
