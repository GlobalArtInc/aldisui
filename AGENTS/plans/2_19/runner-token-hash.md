...
- **Dropping the plaintext `token` column.** Not part of this plan. The
  column stays in the schema indefinitely so older Aldis binaries
...
1. **Existing registered runners** continue to authenticate with the token
   ...
   disk are untouched.
2. **Older Aldis server binaries** sharing the same database (e.g.
...
- Fresh install on each dialect (MySQL, Postgres, SQLite): register a
  runner via `aldis runner register`, confirm the runner polls
