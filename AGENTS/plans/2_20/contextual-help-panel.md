...
Add contextual help to Aldis UI: a `?` icon next to a UI element opens a panel on
the right showing help for that element. The help content must be **built into the
Aldis build** and work **with no internet access** on the user's side.
...
### What already exists in Aldis

Four findings that make this much cheaper than expected:

1. **The docs repo is already a git submodule.** `.gitmodules` maps
   `github.com:GlobalArtInc/aldisui-docs` → `docs/`. The markdown behind
   docs.aldisui.com is already present at `docs/docs/**/*.md` at build time.
...
- Absolute links that rot: `web/src/views/Runners.vue:374` points at
  `https://github.com/GlobalArtInc/aldisui#set-up-a-server`, but
...
|---|---|---|
| Remote images (`aldisui.com/uploads/...`, githubusercontent) | 37 | **Break offline.** |
...
| D. Fetch from docs.aldisui.com at runtime | Rejected — violates the offline requirement. |
...
1. **Content source → dedicated help snippets in the docs repo.** A new
   `help/` folder in `aldis-docs` with short, field-level snippets, each with a
...
- `docs/help/` (in `aldis-docs`) — **new** snippet source.
...
- Rewrite internal links (`./foo.md`, `/user-guide/x`) to absolute
  docs.aldisui.com URLs — these are the optional "Learn more" links, expected to
...
In `aldis-docs`: create `help/`, document the frontmatter contract in
