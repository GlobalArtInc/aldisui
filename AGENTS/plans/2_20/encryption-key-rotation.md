...
Aldis encrypts the most sensitive data it holds with a single
...
1. **It lives inline.** The key sat directly in the config file or in the
   `ALDIS_ACCESS_KEY_ENCRYPTION` environment variable — reachable via
...
aldis vault rekey     # 3. re-encrypt access keys + jwt_signing_key to the primary
aldis vault check     # 4. wait until everything reports the primary
...
(KCV or `HMAC-SHA256(key, "aldis-key-id")` are equivalent conservative
...
1. **No `encryption.keys_file`** → `access_key_encryption` (and
   `ALDIS_ACCESS_KEY_ENCRYPTION`) drives the access keyring exactly as before.
