...
// UserExternalIdentity links a Aldis user to an identity at an external
...
- Modify: `config.schema.yaml` — regenerate via the `aldis-config-schema` skill after the code change
...
Invoke the `aldis-config-schema` skill (it derives `config.schema.yaml` from `util.ConfigType`). In the field description mention that provider key `ldap` is reserved for the LDAP integration.
...
// resolveExternalUser maps an external identity to a Aldis user:
