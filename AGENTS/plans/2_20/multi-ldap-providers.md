...
- After config struct changes, regenerate `config.schema.yaml` (see the `aldis-config-schema` skill) — folded into Task 8.
...
Regenerate `config.schema.yaml` for the new `ldap_providers` field — follow the `aldis-config-schema` skill (map of `LdapProvider` objects: `display_name`, `server`, `needtls`, `binddn`, `bindpassword`, `searchdn`, `searchfilter`, `mappings`, `order`; note in the description that the key `ldap` is reserved for the legacy flat config).
