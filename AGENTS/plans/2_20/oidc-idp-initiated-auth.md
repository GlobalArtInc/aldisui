# IdP-initiated OIDC authentication implementation plan for Aldis UI

Add support for **IdP-initiated** sign-in to the existing OIDC implementation, so a user can start from the identity
provider (Okta dashboard, Azure My Apps, Keycloak / Authentik app launcher, etc.), click the Aldis tile, and land
logged-in — without first visiting the Aldis login page.

## 1. Concept

Today Aldis only supports the **SP-initiated** (a.k.a. RP-initiated) flow:

1. User opens Aldis → `/api/auth/oidc/{provider}/login` (`oidcLogin`, `api/login.go:482`).
2. Aldis generates a CSRF `state` cookie (`generateStateOauthCookie`, `api/login.go:522`) and redirects the browser
...
5. Aldis exchanges the `code`, verifies the ID token, resolves/creates the user, creates a session.

In the **IdP-initiated** flow the journey starts at the IdP, so step 1 never happens — there is no `state` cookie and
no Aldis-controlled request. OIDC defines two ways to deal with this:
...
- **(B) Unsolicited response** — the IdP POSTs an `id_token` directly to a Aldis endpoint (`response_mode=form_post`,
...
| Config schema | `config.schema.yaml` (regenerate via `aldis-config-schema` skill) |
...
After editing the struct, **regenerate `config.schema.yaml`** with the `aldis-config-schema` skill.
...
No login-page change is required — IdP-initiated starts outside Aldis. OIDC providers are config-file driven (no
admin UI today), so:
