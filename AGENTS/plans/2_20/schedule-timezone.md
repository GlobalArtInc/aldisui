...
(`schedule.timezone` config / `ALDIS_SCHEDULE_TIMEZONE` env, default `UTC`).
...
- The info alert about `ALDIS_SCHEDULE_TIMEZONE` should mention it is only
...
- **Docs** — update the schedules page in `docs/` (timezone field, `CRON_TZ=`
  prefix, precedence rules) and the `ALDIS_SCHEDULE_TIMEZONE` description
