...
**Date:** 2026-06-04
**Scope:** Go backend of Aldis (`globalartinc/aldis`).
...
**Good news first** (a dedicated agent traced the whole inventory lifecycle): Aldis treats the inventory as an *
...
`LocalJob_inventory.go:92-98`). Per-host connection fan-out is Ansible's concern (`forks`), not Aldis's. So the "
5000 syscalls / O(hosts²)" class of bug **does not exist** on the Aldis side.
...
- **Local file inventory:** cheapest — Aldis only joins a path; Ansible reads the file (`LocalJob.go:352-357`).
...
- **Unlimited default parallelism + per-task `Sleep` (Finding 10).** `MaxParallelTasks` default `9999` (
  ...
  *Fix:* a bounded worker pool / aldis with a sane default instead of `9999`; drop the unconditional `time.Sleep`.
