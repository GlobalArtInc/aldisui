...
> Deep research into the Aldis Go backend to locate performance and scalability
...
aldis at a sane default), bound the DB pool (DB-8), and drop the unconditional `Sleep`.
...
> **Headline:** Aldis treats the inventory as an **opaque blob**. It is never parsed,
...
| **Local file** (`InventoryFile`)                  | none — Ansible reads from the repo; Aldis only joins a path | nothing in Go                                   |
...
19. **Real admission control** for the unlimited-parallelism default — a bounded worker pool /
    aldis sized sanely (TP-7), pairing the convenient default with backpressure.
