...
environment variable `ALDIS_DEBUG_FILTER` (or the `--debug-filter` flag), mirroring the
...
- Refactored `PersistentPreRun` so it no longer early-returns when the log level is unset
  ...
  `initDebugFilter()`.
- New `initDebugFilter()`:
    - Resolves spec from `--debug-filter`, falling back to `ALDIS_DEBUG_FILTER`.
...
```bash
ALDIS_LOG_LEVEL=DEBUG ALDIS_DEBUG_FILTER=runner aldis server
ALDIS_LOG_LEVEL=DEBUG ALDIS_DEBUG_FILTER='task_*' aldis server
ALDIS_LOG_LEVEL=DEBUG ALDIS_DEBUG_FILTER='*,-db' aldis server
# or via flags:
aldis server --log-level DEBUG --debug-filter 'runner,git'
```
...
- **Syslog filtering** — implemented via `debuglog.FilteringHook` in `cli/cmd/syslog.go`
  ...
  the same `ALDIS_DEBUG_FILTER` / `--debug-filter` spec as stdout.
