...
- **`util/config.go`** — new `RunnersConfig` section (json key `runners`),
  **separate from `RunnerConfig`**: server-side fleet settings use the
  `ALDIS_RUNNERS_*` env prefix; `ALDIS_RUNNER_*` stays for the runner
  process itself.
    - `offline_timeout_sec` — default **120** — `ALDIS_RUNNERS_OFFLINE_TIMEOUT_SEC`
    - `task_fail_timeout_sec` — default **420** — `ALDIS_RUNNERS_TASK_FAIL_TIMEOUT_SEC`
    - `reconcile_interval_sec` — default **30** — `ALDIS_RUNNERS_RECONCILE_INTERVAL_SEC`
