...
1) Set up `GOPATH`
   ...
   * Create required directory and switch to it:
      ...
      ```bash
      mkdir -p $GOPATH/src/github.com/GlobalArtInc
      cd $GOPATH/src/github.com/GlobalArtInc
      ```

2) Clone aldis

   ```
   git clone git@github.com:GlobalArtInc/aldisui.git && cd aldisui
   ```
...
4) Create database if you want to use MySQL or PostgreSQL (Aldis also supports SQLite, which does not require
   additional setup)
   ...
   ```
   echo "create database aldis;" | mysql -uroot -p
   ```
...
2) Install Dredd globally
    ...
    npm install -g dredd
    ```
3) Create `./dredd/config.json` for Dredd. It must contain database connection same as used in Aldis server.
    ...
    ```json
   {
    ...
    ```
4) Start Aldis server (add `--config` option if required):
...
Aldis uses [logrus](https://github.com/sirupsen/logrus) for structured logging.
...
| Flag / env var                        | Values                                             |
|---------------------------------------|----------------------------------------------------|
| `--log-level` / `ALDIS_LOG_LEVEL` | `DEBUG`, `INFO`, `WARN`, `ERROR`, `FATAL`, `PANIC` |

```bash
ALDIS_LOG_LEVEL=DEBUG ./bin/aldis server --config ./config.json
```
...
|---------------------------------------------|-----------------------------|
| `--debug-filter` / `ALDIS_DEBUG_FILTER` | `runner`, `task_*`, `*,-db` |
...
ALDIS_LOG_LEVEL=DEBUG ALDIS_DEBUG_FILTER=runner ./bin/aldis server

# All task-related namespaces except database tracing
ALDIS_LOG_LEVEL=DEBUG ALDIS_DEBUG_FILTER='task_*,*,-db' ./bin/aldis server
```
...
## Manual testing with using Aldis MCP and Cursor agent

1. Install Aldis MCP

   ```bash
   pipx install aldis-mcp
   ```

   Upgrade:

   ```bash
   pipx upgrade aldis-mcp
   ```
   ...
   ```json
	{
	  "mcpServers": {
	    "aldis": {
	      ...
	      "args": [],
	      "env": {
	        "ALDIS_URL": "http://localhost:3000",
	        "ALDIS_API_TOKEN": "<TOKEN>"
	      }
	    }
	  }
	}
