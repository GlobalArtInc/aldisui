...
func TestRequestOriginHost(t *testing.T) {
	t.Run("from Origin header", func(t *testing.T) {
		...
		r.Header.Set("Origin", "https://aldis.example.com")

		host, ok := requestOriginHost(r)
		assert.True(t, ok)
		assert.Equal(t, "aldis.example.com", host)
	})

	t.Run("falls back to Referer", func(t *testing.T) {
		r := httptest.NewRequest(http.MethodPost, "/api/users/1/password", nil)
		r.Header.Set("Referer", "https://aldis.example.com/project/1")
	...
	t.Run("Origin takes precedence over Referer", func(t *testing.T) {
		...
		r.Header.Set("Referer", "https://aldis.example.com/")
...
func TestCsrfProtectionMiddleware(t *testing.T) {
	...
	webHost, err := url.Parse("https://aldis.example.com")
	...
	tests := []struct {
		...
		},
		{
			...
			host:         "aldis.example.com",
			origin:       "https://aldis.example.com",
			wantStatus:   http.StatusNoContent,
			wantForwPass: true,
		},
		{
