...
import (
	...
	"github.com/GlobalArtInc/aldisui/pkg/conv"
	"github.com/GlobalArtInc/aldisui/services/server"
	task2 "github.com/GlobalArtInc/aldisui/services/tasks"
...
func (c *IntegrationController) ReceiveIntegration(w http.ResponseWriter, r *http.Request) {
	...
	for _, integration := range integrations {
		...
		if task != nil {
			w.Header().Add("X-Aldis-Task-ID", strconv.Itoa(task.ID))
			w.Header().Add("X-Aldis-Template-ID", strconv.Itoa(task.TemplateID))
			w.Header().Add("X-Aldis-Project-ID", strconv.Itoa(task.ProjectID))

			if task.IntegrationID != nil {
				w.Header().Add("X-Aldis-Integration-ID", strconv.Itoa(*task.IntegrationID))
			}

			if task.InventoryID != nil {
				w.Header().Add("X-Aldis-Inventory-ID", strconv.Itoa(*task.InventoryID))
			}
		}
	}
