...
func createSession(w http.ResponseWriter, r *http.Request, user db.User, oidc bool) {
	...
	encoded, err := util.Cookie.Encode("aldis", map[string]any{
		"user":    user.ID,
	...
	http.SetCookie(w, &http.Cookie{
		...
		// so following a link into Aldis keeps the user logged in.
		SameSite: http.SameSiteLaxMode,
		// Secure is only enforced when Aldis is served over HTTPS, so that
		// it can still be used without TLS inside private networks.
		Secure: isSecureWebHost(),
	})
}

// isSecureWebHost reports whether Aldis's public web host uses HTTPS, in
...
func generateStateOauthCookie(w http.ResponseWriter, returnPath string, link bool) string {
	...
	// Secure flag is not set to allow Aldis to be used without HTTPS inside private networks
	cookie := http.Cookie{
		Name:     "oauthstate",
		Value:    state.Csrf,
		Expires:  expiration,
