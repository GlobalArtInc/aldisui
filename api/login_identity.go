...
// ldapProfileMatchesAldisUser checks that the LDAP directory profile
// belongs to the Aldis account being linked. A successful bind only proves
// ownership of the LDAP credentials, not that they correspond to this account.
func ldapProfileMatchesAldisUser(ldapUser, aldisUser db.User) bool {
	if ldapUser.Email != "" && strings.EqualFold(ldapUser.Email, aldisUser.Email) {
		return true
	}
	if ldapUser.Username != "" && strings.EqualFold(ldapUser.Username, aldisUser.Username) {
		return true
	}
	return false
}
