...
func TestIsSecureWebHost(t *testing.T) {
	...
	tests := []struct {
		...
		{"https host is secure", "https://aldis.example.com", true},
		{"http host is not secure", "http://aldis.example.com:3000", false},
...
func TestOidcSuccessRedirectURL(t *testing.T) {
	tests := []struct {
		...
		{"web host with sub path", "http://localhost:3000/aldis", "/project/1", "http://localhost:3000/aldis/project/1"},
	}
...
func TestOidcSuccessRedirectURL_NotRelative(t *testing.T) {
	callback, err := url.Parse("http://aldis.example.com/api/auth/oidc/pocketid/redirect")
	require.NoError(t, err)
	...
	assert.Equal(t,
		"http://aldis.example.com/auth/login",
