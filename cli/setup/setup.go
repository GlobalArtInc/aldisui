...
4. Set up initial aldis user & password

`

func InteractiveRunnerSetup(conf *util.ConfigType) {

	askValue("Aldis server URL", "", &conf.WebHost)
...
`
	...
	defaultPlaybookPath := filepath.Join(os.TempDir(), "aldis")
	askValue("Playbook path", defaultPlaybookPath, &conf.TmpPath)
	conf.TmpPath = filepath.Clean(conf.TmpPath)

	askValue("Public URL (optional, example: https://example.com/aldis)", "", &conf.WebHost)
	...
	if conf.EmailAlert {
		...
		askValue("Mail sender address", "aldis@localhost", &conf.EmailSender)
	}
...
func scanMySQL(conf *util.ConfigType) {
	...
	askValue("db Name", "aldis", &conf.MySQL.DbName)
}
...
func scanPostgres(conf *util.ConfigType) {
	...
	askValue("db Name", "aldis", &conf.Postgres.DbName)
...
func SaveConfig(config IConfig, defaultFilename string, requiredConfigPath string) (configPath string) {
	...
	if requiredConfigPath == "" {
		...
		if err != nil {
			...
			if err != nil {
				// Final fallback
				configDirectory = "/etc/aldis"
			}
			configDirectory = filepath.Join(configDirectory, "aldis")
		}
