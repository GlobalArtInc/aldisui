...
different setups of Aldis UI together. Below you can find some example
combinations.
...
If you want to try the remote runner functionality of Aldis you could just
add this snippet to get a runner up and connected to aldis. Similar to the
...
2. **Per-runner registration token** (`smrs_…`) — create an *unregistered* runner in the
   ...
   one-time registration token for that runner and pass it to `aldis runner register`.
