...
If you want to use [docker-compose][dockercompose] to start Aldis you could
...
* `DOCKER_ORG`: Define a custom organization for the image, defaults to `aldisui`
* `DOCKER_SERVER`: Define a different name for the server image, defaults to `aldis`
...
tools and binaries that are required to run Aldis. Here we are using
...
[dockerhub]: https://hub.docker.com/r/globalartinc/aldis
