...
import (
	...
	"github.com/GlobalArtInc/aldisui/db_lib"
...
func (t *LocalExecutor) getEnvironmentENV() (res []string, err error) {
	...
	if t.JWT != "" {
		res = append(res, fmt.Sprintf("ALDIS_JWT=%s", t.JWT))
	}
...
func (t *LocalExecutor) getShellEnvironmentExtraENV(username string, incomingVersion *string) (extraShellVars []string) {
	...
	for taskDetail, taskDetailValue := range taskDetails {
		envVarName := fmt.Sprintf("ALDIS_TASK_DETAILS_%s", strings.ToUpper(taskDetail))
...
func (t *LocalExecutor) Prepare(username string, incomingVersion *string, alias string) (err error) {
	...
	switch t.Template.App {
	case db.AppAnsible:
		// Aldis vars / task details were already passed
	...
	if t.Template.Type != db.TemplateTask {
		...
		environmentVariables = append(environmentVariables, fmt.Sprintf("ALDIS_TASK_TYPE=%s", t.Template.Type))

		if incomingVersion != nil {
			environmentVariables = append(
				environmentVariables,
				fmt.Sprintf("ALDIS_TASK_INCOMING_VERSION=%s", *incomingVersion))
		}

		if t.Template.Type == db.TemplateBuild && t.Task.Version != nil {
			environmentVariables = append(
				environmentVariables,
				fmt.Sprintf("ALDIS_TASK_TARGET_VERSION=%s", *t.Task.Version))
		}
	}
