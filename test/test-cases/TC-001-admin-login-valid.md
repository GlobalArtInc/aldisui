...
* Aldis instance freshly started with environment variables
  `ALDIS_ADMIN=admin`, `ALDIS_ADMIN_PASSWORD=changeme`,
  `ALDIS_ADMIN_NAME=Admin`, `ALDIS_ADMIN_EMAIL=admin@localhost`.
...
* A `aldis` session cookie is present, has `HttpOnly` set, and `Secure` when
  served over HTTPS.
