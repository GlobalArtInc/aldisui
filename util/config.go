...
// WebHostURL is the public route to the aldis server
...
type DbConfig struct {
	...
	Hostname string            `json:"host,omitempty" env:"ALDIS_DB_HOST" default:"0.0.0.0"`
	Username string            `json:"user,omitempty" env:"ALDIS_DB_USER"`
	Password string            `json:"pass,omitempty" env:"ALDIS_DB_PASS,sensitive"`
	DbName   string            `json:"name,omitempty" env:"ALDIS_DB" default:"aldis"`
	Options  map[string]string `json:"options,omitempty" env:"ALDIS_DB_OPTIONS"`
}

type LdapMappings struct {
	DN   string `json:"dn" env:"ALDIS_LDAP_MAPPING_DN" default:"dn"`
	Mail string `json:"mail" env:"ALDIS_LDAP_MAPPING_MAIL" default:"mail"`
	UID  string `json:"uid" env:"ALDIS_LDAP_MAPPING_UID" default:"uid"`
	CN   string `json:"cn" env:"ALDIS_LDAP_MAPPING_CN" default:"cn"`
}
...
type RunnerConnectionConfig struct {
	// ServerCACertFile is a PEM bundle used to verify the Aldis
	...
	ServerCACertFile string `json:"server_ca_cert_file,omitempty" env:"ALDIS_RUNNER_SERVER_CA_CERT_FILE"`
	// SkipTLSVerify disables server certificate verification entirely.
	// This is insecure (vulnerable to MITM) — use only for testing.
	SkipTLSVerify bool `json:"skip_tls_verify,omitempty" env:"ALDIS_RUNNER_SKIP_TLS_VERIFY"`
}
...
type ExecutorConfig struct {
	Type   ExecutorType       `json:"type" default:"local" env:"ALDIS_RUNNER_EXECUTOR_TYPE"`
	K8s    RunnerK8sConfig    `json:"k8s"`
	Docker RunnerDockerConfig `json:"docker"`
}

type RunnerConfig struct {
	RegistrationToken     string `json:"-" env:"ALDIS_RUNNER_REGISTRATION_TOKEN"`
	RegistrationTokenFile string `json:"registration_token_file,omitempty" env:"ALDIS_RUNNER_REGISTRATION_TOKEN_FILE"`
	Token                 string `json:"token,omitempty" env:"ALDIS_RUNNER_TOKEN,sensitive"`
	TokenFile             string `json:"token_file,omitempty" env:"ALDIS_RUNNER_TOKEN_FILE"`

	// OneOff indicates than runner runs only one job and exit. It is very useful for dynamic runners.
	// How it works?
	// Example:
	// 1) User starts the task.
	// 2) Aldis found runner for task and calls runner's webhook if it provided.
	// 3) Your server or lambda handling the call and starts the one-off runner.
	// 4) The runner connects to the Aldis server and handles the enqueued task(s).
	OneOff bool `json:"one_off,omitempty" env:"ALDIS_RUNNER_ONE_OFF"`

	Enabled          bool     `json:"enabled,omitempty" env:"ALDIS_RUNNER_ENABLED"`
	Webhook          string   `json:"webhook,omitempty" env:"ALDIS_RUNNER_WEBHOOK"`
	Name             string   `json:"name,omitempty" env:"ALDIS_RUNNER_NAME"`
	Tags             []string `json:"tags,omitempty" env:"ALDIS_RUNNER_TAGS"`
	MaxParallelTasks int      `json:"max_parallel_tasks,omitempty" default:"9999" env:"ALDIS_RUNNER_MAX_PARALLEL_TASKS"`
	ProjectID        *int     `json:"project_id,omitempty" env:"ALDIS_RUNNER_PROJECT_ID"`

	Connection *RunnerConnectionConfig `json:"connection,omitempty"`

	Executor *ExecutorConfig `json:"executor,omitempty" env:"ALDIS_RUNNER_EXECUTOR"`
}
...
type RunnerK8sConfig struct {
	...
	KubeconfigPath string `json:"kubeconfig,omitempty" env:"ALDIS_RUNNER_K8S_KUBECONFIG"`

	// Namespace is where ephemeral task Pods are created.
	Namespace string `json:"namespace,omitempty" default:"aldis" env:"ALDIS_RUNNER_K8S_NAMESPACE"`
	...
	Image string `json:"image,omitempty" default:"globalartinc/aldis-job:latest" env:"ALDIS_RUNNER_K8S_IMAGE"`

	// HelperImage is the image used for the git-clone init container (Phase 3+).
	HelperImage string `json:"helper_image,omitempty" default:"globalartinc/aldis-helper:latest" env:"ALDIS_RUNNER_K8S_HELPER_IMAGE"`

	// ServiceAccount that task Pods run under. Defaults to the namespace's default SA.
	ServiceAccount string `json:"service_account,omitempty" default:"default" env:"ALDIS_RUNNER_K8S_SERVICE_ACCOUNT"`

	// PullSecrets is a comma-separated list of imagePullSecrets attached to each Pod.
	PullSecrets string `json:"pull_secrets,omitempty" env:"ALDIS_RUNNER_K8S_PULL_SECRETS"`
	...
	PollIntervalSeconds int `json:"poll_interval_seconds,omitempty" default:"3" env:"ALDIS_RUNNER_K8S_POLL_INTERVAL_SECONDS"`

	// CleanupGraceSeconds is the grace period when deleting Pods. Defaults to 30s.
	CleanupGraceSeconds int `json:"cleanup_grace_seconds,omitempty" default:"30" env:"ALDIS_RUNNER_K8S_CLEANUP_GRACE_SECONDS"`
}
...
type RunnerDockerConfig struct {
	...
	// socket are used.
	Host string `json:"host,omitempty" env:"ALDIS_RUNNER_DOCKER_HOST"`

	// TLSVerify enables TLS certificate verification for tcp:// connections.
	TLSVerify bool `json:"tls_verify,omitempty" env:"ALDIS_RUNNER_DOCKER_TLS_VERIFY"`

	// CertPath is the directory holding ca.pem, cert.pem and key.pem for mutual TLS
	// against a remote daemon.
	CertPath string `json:"cert_path,omitempty" env:"ALDIS_RUNNER_DOCKER_CERT_PATH"`

	// Image is the default image used for the build container of each task.
	Image string `json:"image,omitempty" default:"globalartinc/aldis-job:latest" env:"ALDIS_RUNNER_DOCKER_IMAGE"`

	// HelperImage is the image used for the transient git-clone container.
	HelperImage string `json:"helper_image,omitempty" default:"globalartinc/aldis-helper:latest" env:"ALDIS_RUNNER_DOCKER_HELPER_IMAGE"`

	// Network is the Docker network the build container joins. Defaults to "bridge".
	Network string `json:"network,omitempty" default:"bridge" env:"ALDIS_RUNNER_DOCKER_NETWORK"`

	// PullPolicy controls image pulling: always, if-not-present or never.
	PullPolicy string `json:"pull_policy,omitempty" default:"if-not-present" env:"ALDIS_RUNNER_DOCKER_PULL_POLICY"`

	// CPULimit, when > 0, caps the build container CPU (passed as --cpus).
	CPULimit float64 `json:"cpu_limit,omitempty" env:"ALDIS_RUNNER_DOCKER_CPU_LIMIT"`

	// MemoryLimit, when non-empty, caps the build container memory (e.g. "2g").
	MemoryLimit string `json:"memory_limit,omitempty" env:"ALDIS_RUNNER_DOCKER_MEMORY_LIMIT"`

	// PollIntervalSeconds controls how often container status is polled. Defaults to 2s.
	PollIntervalSeconds int `json:"poll_interval_seconds,omitempty" default:"2" env:"ALDIS_RUNNER_DOCKER_POLL_INTERVAL_SECONDS"`

	// CleanupGraceSeconds is the timeout passed to docker stop. Defaults to 30s.
	CleanupGraceSeconds int `json:"cleanup_grace_seconds,omitempty" default:"30" env:"ALDIS_RUNNER_DOCKER_CLEANUP_GRACE_SECONDS"`

	// Privileged runs the build container with --privileged. Dangerous; off by default.
	Privileged bool `json:"privileged,omitempty" env:"ALDIS_RUNNER_DOCKER_PRIVILEGED"`
}
...
// process itself: server-side fleet settings use the ALDIS_RUNNERS_* env
// prefix, runner-process settings use ALDIS_RUNNER_*.
type RunnersConfig struct {
	...
	OfflineTimeoutSec int `json:"offline_timeout_sec,omitempty" default:"120" env:"ALDIS_RUNNERS_OFFLINE_TIMEOUT_SEC"`
	...
	TaskFailTimeoutSec int `json:"task_fail_timeout_sec,omitempty" default:"420" env:"ALDIS_RUNNERS_TASK_FAIL_TIMEOUT_SEC"`

	// ReconcileIntervalSec is how often the server scans dispatched tasks
	// against runner liveness.
	ReconcileIntervalSec int `json:"reconcile_interval_sec,omitempty" default:"30" env:"ALDIS_RUNNERS_RECONCILE_INTERVAL_SEC"`

	// RunnerRegistrationToken is deprecated, use Runners field instead of it.
	RegistrationToken string `json:"registration_token,omitempty" env:"ALDIS_RUNNER_REGISTRATION_TOKEN"`

	DefaultGlobalRunnersMode DefultGlobalRunnerMode `json:"default_global_runners_mode" env:"ALDIS_DEFAULT_GLOBAL_RUNNERS_MODE"`
}

type TLSConfig struct {
	Enabled          bool   `json:"enabled" env:"ALDIS_TLS_ENABLED"`
	CertFile         string `json:"cert_file" env:"ALDIS_TLS_CERT_FILE"`
	KeyFile          string `json:"key_file" env:"ALDIS_TLS_KEY_FILE"`
	HTTPRedirectAddr string `json:"http_redirect_addr,omitempty" env:"ALDIS_TLS_HTTP_REDIRECT_ADDR"`
	HTTPRedirectPort *int   `json:"http_redirect_port,omitempty" env:"ALDIS_TLS_HTTP_REDIRECT_PORT"`
}

type TotpConfig struct {
	Enabled       bool   `json:"enabled" env:"ALDIS_TOTP_ENABLED"`
	AllowRecovery bool   `json:"allow_recovery" env:"ALDIS_TOTP_ALLOW_RECOVERY"`
	Issuer        string `json:"app_name" env:"ALDIS_TOTP_ISSUER"`
}

type EventLogType struct {
	Format  string             `json:"format,omitempty" env:"ALDIS_EVENT_LOG_FORMAT"`
	Enabled bool               `json:"enabled" env:"ALDIS_EVENT_LOG_ENABLED"`
	Logger  *lumberjack.Logger `json:"logger,omitempty" env:"ALDIS_EVENT_LOGGER"`
}
...
type TaskLogType struct {
	Enabled      bool               `json:"enabled" env:"ALDIS_TASK_LOG_ENABLED"`
	Format       string             `json:"format,omitempty" env:"ALDIS_TASK_LOG_FORMAT"`
	Logger       *lumberjack.Logger `json:"logger,omitempty" env:"ALDIS_TASK_LOGGER"`
	ResultLogger *lumberjack.Logger `json:"result_logger,omitempty" env:"ALDIS_TASK_RESULT_LOGGER"`
}
...
type SyslogConfig struct {
	Enabled bool         `json:"enabled" env:"ALDIS_SYSLOG_ENABLED"`
	Network string       `json:"network,omitempty" env:"ALDIS_SYSLOG_NETWORK"`
	Address string       `json:"address,omitempty" env:"ALDIS_SYSLOG_ADDRESS"`
	Tag     string       `json:"tag,omitempty" env:"ALDIS_SYSLOG_TAG"`
	Format  SyslogFormat `json:"format,omitempty" env:"ALDIS_SYSLOG_FORMAT"`
}

type MetricsConfig struct {
	Enabled  bool   `json:"enabled" env:"ALDIS_METRICS_ENABLED"`
	Username string `json:"username,omitempty" env:"ALDIS_METRICS_USERNAME"`
	Password string `json:"password,omitempty" env:"ALDIS_METRICS_PASSWORD,sensitive"`
}

type ConfigProcess struct {
	User       string  `json:"user,omitempty" env:"ALDIS_PROCESS_USER"`
	UID        *uint32 `json:"uid,omitempty" env:"ALDIS_PROCESS_UID"`
	Chroot     string  `json:"chroot,omitempty" env:"ALDIS_PROCESS_CHROOT"`
	GID        *uint32 `json:"gid,omitempty" env:"ALDIS_PROCESS_GID"`
	NoNewPrivs bool    `json:"no_new_privs,omitempty" env:"ALDIS_PROCESS_NO_NEW_PRIVS"`
...
type ConfigAppNamespaces struct {
	...
	User bool `json:"user,omitempty" env:"ALDIS_PROCESS_APP_NS_USER"`
	// Mount hides host mount points such as secret tmpfs (CLONE_NEWNS).
	Mount bool `json:"mount,omitempty" env:"ALDIS_PROCESS_APP_NS_MOUNT"`
	// PID hides host processes from child apps (CLONE_NEWPID).
	PID bool `json:"pid,omitempty" env:"ALDIS_PROCESS_APP_NS_PID"`
	// IPC isolates SysV IPC and POSIX message queues (CLONE_NEWIPC).
	IPC bool `json:"ipc,omitempty" env:"ALDIS_PROCESS_APP_NS_IPC"`
	// UTS isolates hostname and domain (CLONE_NEWUTS).
	UTS bool `json:"uts,omitempty" env:"ALDIS_PROCESS_APP_NS_UTS"`
}

type ScheduleConfig struct {
	Timezone string `json:"timezone,omitempty" env:"ALDIS_SCHEDULE_TIMEZONE" default:"UTC"`
}

type DebuggingConfig struct {
	ApiDelay     string `json:"api_delay,omitempty" env:"ALDIS_API_DELAY"`
	PprofDumpDir string `json:"pprof_dump_dir,omitempty" env:"ALDIS_PPROF_DUMP_DIR"`
}

type HARedisConfig struct {
	Addr          string `json:"addr,omitempty" env:"ALDIS_HA_REDIS_ADDR"`
	DB            int    `json:"db,omitempty" env:"ALDIS_HA_REDIS_DB"`
	Pass          string `json:"pass,omitempty" env:"ALDIS_HA_REDIS_PASS,sensitive"`
	User          string `json:"user,omitempty" env:"ALDIS_HA_REDIS_USER"`
	TLS           bool   `json:"tls,omitempty" env:"ALDIS_HA_REDIS_TLS"`
	TLSSkipVerify bool   `json:"tls_skip_verify,omitempty" env:"ALDIS_HA_REDIS_TLS_SKIP_VERIFY"`
}

type HAConfig struct {
	Enabled bool           `json:"enabled" env:"ALDIS_HA_ENABLED"`
	NodeID  string         `json:"node_id,omitempty" env:"ALDIS_HA_NODE_ID"` // auto-generated if empty
...
type TeamsConfig struct {
	InvitesEnabled  bool           `json:"invites_enabled,omitempty" env:"ALDIS_TEAMS_INVITES_ENABLED"`
	InviteType      TeamInviteType `json:"invite_type,omitempty" env:"ALDIS_TEAMS_INVITE_TYPE" default:"username"`
	MembersCanLeave bool           `json:"members_can_leave,omitempty" env:"ALDIS_TEAMS_MEMBERS_CAN_LEAVE"`
}

type ConfigDirs struct {
	Secrets         string `json:"secrets,omitempty" env:"ALDIS_SECRETS_PATH" default:"/tmp/aldis"`
	Repos           string `json:"repos,omitempty" env:"ALDIS_REPOS_DIR"`
	SSHAgentSockets string `json:"ssh_agent_sockets,omitempty" env:"ALDIS_SSH_AGENT_SOCKETS_DIR" default:"/tmp/aldis"`
}

// JWTConfig issuance for task executions (used by playbooks to authenticate to
type JWTConfig struct {
	Enabled    bool   `json:"enabled,omitempty" env:"ALDIS_JWT_ENABLED"`
	Issuer     string `json:"issuer,omitempty" env:"ALDIS_JWT_ISSUER"`
	DefaultTTL string `json:"default_ttl,omitempty" env:"ALDIS_JWT_DEFAULT_TTL" default:"1h"`
	MaxTTL     string `json:"max_ttl,omitempty" env:"ALDIS_JWT_MAX_TTL" default:"24h"`
}
...
type EncryptionConfig struct {
	...
	KeysFile string `json:"keys_file,omitempty" env:"ALDIS_ENCRYPTION_KEYS_FILE"`
	...
	KeysPollInterval string `json:"keys_poll_interval,omitempty" env:"ALDIS_ENCRYPTION_KEYS_POLL_INTERVAL" default:"15s"`
}
...
type SshConfig struct {
	...
	ConfigPath string `json:"config_path,omitempty" env:"ALDIS_SSH_PATH"`
	...
	// empty, Aldis uses a persistent trust-on-first-use file under TmpPath
	...
	KnownHostsFile string `json:"known_hosts_file,omitempty" env:"ALDIS_SSH_KNOWN_HOSTS_FILE"`
...
type ConfigType struct {
	...
	Dialect string `json:"dialect,omitempty" default:"sqlite" rule:"^mysql|postgres|sqlite$" env:"ALDIS_DB_DIALECT"`

	// Format `:port_num` eg, :3000
	// if : is missing it will be corrected
	Port string     `json:"port,omitempty" default:":3000" rule:"^:?([0-9]{1,5})$" env:"ALDIS_PORT"`
	...
	// defaults to empty
	Interface string `json:"interface,omitempty" env:"ALDIS_INTERFACE"`

	// aldis stores ephemeral projects here
	TmpPath string `json:"tmp_path,omitempty" default:"/tmp/aldis" env:"ALDIS_TMP_PATH"`
	...
	SecretsPath string `json:"secrets_path,omitempty" env:"ALDIS_SECRETS_PATH"`
	...
	HomeDirMode string `json:"home_dir_mode,omitempty" rule:"^(user_home|project_home|template_dir)?$" env:"ALDIS_HOME_DIR_MODE" default:"template_dir"`

	// SshConfigPath is a path to the custom SSH config file.
	// Default path is ~/.ssh/config.
	SshConfigPath string `json:"ssh_config_path,omitempty" env:"ALDIS_SSH_PATH"`

	Ssh *SshConfig `json:"ssh"`

	GitClientId string `json:"git_client,omitempty" rule:"^go_git|cmd_git$" env:"ALDIS_GIT_CLIENT" default:"cmd_git"`
	...
	GitAttempts int `json:"git_attempts,omitempty" env:"ALDIS_GIT_ATTEMPTS" default:"4"`

	// web host
	WebHost string `json:"web_host,omitempty" env:"ALDIS_WEB_ROOT"`

	// cookie hashing & encryption
	CookieHash       string `json:"cookie_hash,omitempty" env:"ALDIS_COOKIE_HASH,sensitive"`
	CookieEncryption string `json:"cookie_encryption,omitempty" env:"ALDIS_COOKIE_ENCRYPTION,sensitive"`
	...
	AccessKeyEncryption string `json:"access_key_encryption,omitempty" env:"ALDIS_ACCESS_KEY_ENCRYPTION,sensitive"`
	...
	// access keyring.
	OptionEncryption string `json:"option_encryption,omitempty" env:"ALDIS_OPTION_ENCRYPTION,sensitive"`

	// email alerting
	EmailAlert         bool   `json:"email_alert,omitempty" env:"ALDIS_EMAIL_ALERT"`
	EmailSender        string `json:"email_sender,omitempty" env:"ALDIS_EMAIL_SENDER"`
	EmailHost          string `json:"email_host,omitempty" env:"ALDIS_EMAIL_HOST"`
	EmailPort          string `json:"email_port,omitempty" rule:"^(|[0-9]{1,5})$" env:"ALDIS_EMAIL_PORT"`
	EmailUsername      string `json:"email_username,omitempty" env:"ALDIS_EMAIL_USERNAME"`
	EmailPassword      string `json:"email_password,omitempty" env:"ALDIS_EMAIL_PASSWORD,sensitive"`
	EmailSecure        bool   `json:"email_secure,omitempty" env:"ALDIS_EMAIL_SECURE"`
	EmailTls           bool   `json:"email_tls,omitempty" env:"ALDIS_EMAIL_TLS"`
	EmailTlsMinVersion string `json:"email_tls_min_version,omitempty" default:"1.2" rule:"^(1\\.[0123])$" env:"ALDIS_EMAIL_TLS_MIN_VERSION"`

	// ldap settings
	LdapEnable       bool          `json:"ldap_enable,omitempty" env:"ALDIS_LDAP_ENABLE"`
	LdapBindDN       string        `json:"ldap_binddn,omitempty" env:"ALDIS_LDAP_BIND_DN"`
	LdapBindPassword string        `json:"ldap_bindpassword,omitempty" env:"ALDIS_LDAP_BIND_PASSWORD,sensitive"`
	LdapServer       string        `json:"ldap_server,omitempty" env:"ALDIS_LDAP_SERVER"`
	LdapSearchDN     string        `json:"ldap_searchdn,omitempty" env:"ALDIS_LDAP_SEARCH_DN"`
	LdapSearchFilter string        `json:"ldap_searchfilter,omitempty" env:"ALDIS_LDAP_SEARCH_FILTER"`
	LdapMappings     *LdapMappings `json:"ldap_mappings,omitempty"`
	LdapNeedTLS      bool          `json:"ldap_needtls,omitempty" env:"ALDIS_LDAP_NEEDTLS"`
	...
	LdapTLSSkipVerify bool `json:"ldap_tls_skip_verify,omitempty" env:"ALDIS_LDAP_TLS_SKIP_VERIFY"`
	...
	LdapProviders map[string]LdapProvider `json:"ldap_providers,omitempty" env:"ALDIS_LDAP_PROVIDERS"`

	// Telegram, Slack, Rocket.Chat, Microsoft Teams, DingTalk, and Gotify alerting
	TelegramAlert       bool   `json:"telegram_alert,omitempty" env:"ALDIS_TELEGRAM_ALERT"`
	TelegramChat        string `json:"telegram_chat,omitempty" env:"ALDIS_TELEGRAM_CHAT"`
	TelegramToken       string `json:"telegram_token,omitempty" env:"ALDIS_TELEGRAM_TOKEN,sensitive"`
	SlackAlert          bool   `json:"slack_alert,omitempty" env:"ALDIS_SLACK_ALERT"`
	SlackUrl            string `json:"slack_url,omitempty" env:"ALDIS_SLACK_URL"`
	RocketChatAlert     bool   `json:"rocketchat_alert,omitempty" env:"ALDIS_ROCKETCHAT_ALERT"`
	RocketChatUrl       string `json:"rocketchat_url,omitempty" env:"ALDIS_ROCKETCHAT_URL"`
	MicrosoftTeamsAlert bool   `json:"microsoft_teams_alert,omitempty" env:"ALDIS_MICROSOFT_TEAMS_ALERT"`
	MicrosoftTeamsUrl   string `json:"microsoft_teams_url,omitempty" env:"ALDIS_MICROSOFT_TEAMS_URL"`
	DingTalkAlert       bool   `json:"dingtalk_alert,omitempty" env:"ALDIS_DINGTALK_ALERT"`
	DingTalkUrl         string `json:"dingtalk_url,omitempty" env:"ALDIS_DINGTALK_URL"`
	GotifyAlert         bool   `json:"gotify_alert,omitempty" env:"ALDIS_GOTIFY_ALERT"`
	GotifyUrl           string `json:"gotify_url,omitempty" env:"ALDIS_GOTIFY_URL"`
	GotifyToken         string `json:"gotify_token,omitempty" env:"ALDIS_GOTIFY_TOKEN,sensitive"`

	// oidc settings
	OidcProviders map[string]OidcProvider `json:"oidc_providers,omitempty" env:"ALDIS_OIDC_PROVIDERS"`

	MaxTaskDurationSec  int `json:"max_task_duration_sec,omitempty" env:"ALDIS_MAX_TASK_DURATION_SEC"`
	MaxTasksPerTemplate int `json:"max_tasks_per_template,omitempty" env:"ALDIS_MAX_TASKS_PER_TEMPLATE"`

	// task concurrency
	MaxParallelTasks int `json:"max_parallel_tasks,omitempty" default:"9999" rule:"^[0-9]{1,10}$" env:"ALDIS_MAX_PARALLEL_TASKS"`

	// RunnerRegistrationToken is deprecated, use Runners field instead of it.
	RunnerRegistrationToken string `json:"runner_registration_token,omitempty" env:"ALDIS_RUNNER_REGISTRATION_TOKEN"`

	JWT *JWTConfig `json:"jwt,omitempty"`

	// feature switches
	PasswordLoginDisable bool `json:"password_login_disable,omitempty" env:"ALDIS_PASSWORD_LOGIN_DISABLED"`
	...
	ExternalAuthEmailMatching string `json:"external_auth_email_matching,omitempty" env:"ALDIS_EXTERNAL_AUTH_EMAIL_MATCHING" rule:"^(auto|always|never)?$" default:"auto"`
	NonAdminCanCreateProject  bool   `json:"non_admin_can_create_project,omitempty" env:"ALDIS_NON_ADMIN_CAN_CREATE_PROJECT"`

	// UseRemoteRunner is deprecated. Use Runners field instead of it.
	UseRemoteRunner bool `json:"use_remote_runner,omitempty" env:"ALDIS_USE_REMOTE_RUNNER"`

	Apps map[string]App `json:"apps,omitempty" env:"ALDIS_APPS"`

	EnvVars map[string]string `json:"env_vars,omitempty" env:"ALDIS_ENV_VARS"`

	ForwardedEnvVars []string `json:"forwarded_env_vars,omitempty" env:"ALDIS_FORWARDED_ENV_VARS"`
...
func (conf *ConfigType) GetSecretsPath() string {
	if conf.Dirs.Secrets != "" && conf.Dirs.Secrets != "/tmp/aldis" {
...
type SubscriptionConfig struct {
	...
	Key       string `json:"key,omitempty" db:"-" env:"ALDIS_SUBSCRIPTION_KEY,sensitive"`
	KeyFile   string `json:"key_file,omitempty" db:"-" env:"ALDIS_SUBSCRIPTION_KEY_FILE"`
	ServerURL string `json:"server_url,omitempty" env:"ALDIS_SUBSCRIPTION_SERVER_URL"`
}
...
func ConfigInit(configPath string, noConfigFile bool) (usedConfigPath *string) {
	...
	if Config.Runner != nil && Config.Runner.Token != "" && Config.Runner.TokenFile != "" {
		panic("ALDIS_RUNNER_TOKEN and ALDIS_RUNNER_TOKEN_FILE are mutually exclusive")
	}
...
func loadConfigFile(configPath string) (usedConfigPath *string) {
	if configPath == "" {
		configPath = os.Getenv("ALDIS_CONFIG_PATH")
	}
	...
	if configPath == "" {
		...
		paths := []string{
			...
			"/usr/local/etc/aldis/config.json",
			"/usr/local/etc/aldis/config.yaml",
			"/usr/local/etc/aldis/config.yml",
			"/etc/aldis/config.json",
			"/etc/aldis/config.yaml",
			"/etc/aldis/config.yml",
		}
		for _, p := range paths {
			_, err = os.Stat(p)
			if err != nil {
...
func loadConfigDefaults() {
	...
	if legacySecretsPath != "" && (Config.Dirs.Secrets == "/tmp/aldis" || Config.Dirs.Secrets == "") {
...
// parseEnvTag splits an env tag value like "ALDIS_DB_PASS,sensitive"
...
func exitOnConfigFileError(err error) {
	if err != nil {
		exitOnConfigError("Cannot Find configuration! Use --config parameter to point to a JSON or YAML file generated by `aldis setup`.")
	}
}
...
// FindAldis looks in the PATH for the aldis variable
// if not found it will attempt to find the absolute path of the first
// os argument, the aldis command, and return it
func FindAldis() string {
	cmdPath, _ := exec.LookPath("aldis") //nolint: gas
...
// CheckUpdate uses the GitHub client to check for new tags in the aldis repo
func CheckUpdate() (updateAvailable *github.RepositoryRelease, err error) {
	// fetch releases
	gh := github.NewClient(nil)
	releases, _, err := gh.Repositories.ListReleases(context.TODO(), "aldisui", "aldis", nil)
	if err != nil {
		return
	}
...
func (d *DbConfig) GetDbName() string {
	dbName := os.Getenv("ALDIS_DB_NAME")
	if dbName != "" {
		return dbName
	}
	return d.DbName
...
func (d *DbConfig) GetUsername() string {
	username := os.Getenv("ALDIS_DB_USER")
	if username != "" {
		return username
	}
	return d.Username
...
func (d *DbConfig) GetPassword() string {
	password := os.Getenv("ALDIS_DB_PASS")
	if password != "" {
		return password
	}
	return d.Password
...
func (d *DbConfig) GetHostname() string {
	hostname := os.Getenv("ALDIS_DB_HOST")
	if hostname != "" {
		return hostname
	}
	return d.Hostname
