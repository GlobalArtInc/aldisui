...
func TestLoadEnvironmentToObject_RunnerExecutor(t *testing.T) {
	...
	origExecutor, hadExecutor := os.LookupEnv("ALDIS_RUNNER_EXECUTOR")
	origNetwork, hadNetwork := os.LookupEnv("ALDIS_RUNNER_DOCKER_NETWORK")
	t.Cleanup(func() {
		if hadExecutor {
			_ = os.Setenv("ALDIS_RUNNER_EXECUTOR", origExecutor)
		} else {
			_ = os.Unsetenv("ALDIS_RUNNER_EXECUTOR")
		}
		if hadNetwork {
			_ = os.Setenv("ALDIS_RUNNER_DOCKER_NETWORK", origNetwork)
		} else {
			_ = os.Unsetenv("ALDIS_RUNNER_DOCKER_NETWORK")
		}
	})
	require.NoError(t, os.Setenv("ALDIS_RUNNER_EXECUTOR", `{"type":"docker","docker":{"image":"example.com/job:1"}}`))
	require.NoError(t, os.Setenv("ALDIS_RUNNER_DOCKER_NETWORK", "host"))
...
func TestLoadConfigEnvironmet(t *testing.T) {
	...
	os.Setenv("ALDIS_PORT", envPort)                                 //nolint:errcheck
	os.Setenv("ALDIS_COOKIE_HASH", envCookieHash)                    //nolint:errcheck
	os.Setenv("ALDIS_ACCESS_KEY_ENCRYPTION", envAccessKeyEncryption) //nolint:errcheck
	os.Setenv("ALDIS_MAX_PARALLEL_TASKS", envMaxParallelTasks)       //nolint:errcheck
	os.Setenv("ALDIS_LDAP_NEEDTLS", envLdapNeedTls)                  //nolint:errcheck
	os.Setenv("ALDIS_DB_HOST", envDbHost)                            //nolint:errcheck
...
func TestGetSecretsPath_Default(t *testing.T) {
	Config = NewConfigType()
	loadConfigDefaults()
	assert.Equal(t, "/tmp/aldis", Config.GetSecretsPath())
	require.NotNil(t, Config.Dirs)
	assert.Equal(t, "/tmp/aldis", Config.Dirs.SSHAgentSockets)
}

func TestGetSecretsPath_Env(t *testing.T) {
	Config = NewConfigType()
	orig, existed := os.LookupEnv("ALDIS_SECRETS_PATH")
	t.Cleanup(func() {
		if existed {
			_ = os.Setenv("ALDIS_SECRETS_PATH", orig)
		} else {
			_ = os.Unsetenv("ALDIS_SECRETS_PATH")
		}
	})
	_ = os.Setenv("ALDIS_SECRETS_PATH", "/env/secrets/path")
