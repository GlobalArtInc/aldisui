            ...
            <v-tab-item key="config">
              ...
              <div>
                <pre
                  ...
                  class="pa-2"
                >
aldis runner start --config /path/to/config/file</pre
                >
              </div>
            </v-tab-item>
            <v-tab-item key="setup">
              ...
              <div class="mt-3">
                ...
                <pre
                  ...
                  class="pa-2"
                >
aldis runner start --config ./config.runner.json</pre
                >
              </div>
            </v-tab-item>
            <v-tab-item key="env">
    ...
    >
      ...
      <a
        target="_blank"
        href="https://github.com/GlobalArtInc/aldisui#set-up-a-server"
      >disabled</a
      >.
    </v-alert>
...
export default {
  ...
  computed: {
    ...
    runnerRegisterEnvCommand() {
      return `ALDIS_WEB_ROOT=${this.webHost} \\
ALDIS_RUNNER_REGISTRATION_TOKEN=${(this.newRunner || {}).registration_token} \\
aldis runner register --config ./config.runner.json

aldis runner start --config ./config.runner.json`;
    },
  ...
  "web_host": "${this.webHost || window.location.origin}"
    ...
    runnerRegisterConfigCommand() {
      return `ALDIS_RUNNER_REGISTRATION_TOKEN=${(this.newRunner || {}).registration_token} \\
aldis runner register --config ./config.runner.json
...
aldis runner start --config ./config.runner.json`;
    ...
    runnerRegisterDockerCommand() {
      return `docker run \\
-e ALDIS_WEB_ROOT=${this.webHost} \\
-e ALDIS_RUNNER_REGISTRATION_TOKEN=${(this.newRunner || {}).registration_token} \\
-d globalartinc/aldis-runner:${this.version}`;
    },
...
aldis runner setup --config ./config.runner.json < /tmp/config.runner.stdin`;
    },

    runnerEnvCommand() {
      return `ALDIS_WEB_ROOT=${this.webHost} \\
ALDIS_RUNNER_TOKEN=${(this.newRunner || {}).token} \\
aldis runner start --no-config`;
    },
    ...
    runnerDockerCommand() {
      return `docker run \\
-e ALDIS_WEB_ROOT=${this.webHost} \\
-e ALDIS_RUNNER_TOKEN=${(this.newRunner || {}).token} \\
-d globalartinc/aldis-runner:${this.version}`;
    },
  },
